(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(require,module,exports){
    ...
    var all_measures = convert().measures();
    for (var i = 0; i < all_measures.length; i++) {
        var measure = all_measures[i];
        var opt = document.createElement('option');
        opt.value = measure;
        opt.innerHTML = measure[0].toUpperCase() + measure.substring(1);
        document.getElementById("unit-type").appendChild(opt);
    }
    ...
    $("#unit-type").change(function () {
        fillDropdown($(this));
        convertUnits($("#quant1"), $("#quant2"));
    })

    var select1 = document.getElementById("dropdown1");
    var select2 = document.getElementById("dropdown2");

    function fillDropdown(elem) {
        var measure = elem.val();
        var measure_list = convert().list(measure);
        select1.innerHTML = "";
        select2.innerHTML = "";
        for (var i = 0; i < measure_list.length; i++) {
            var abbr = measure_list[i].abbr;
            var label = measure_list[i].singular + " (" + abbr + ")";
            var opt = document.createElement('option');
            opt.value = abbr;
            opt.innerHTML = label;
    ...
    fillDropdown($("#unit-type"));

    // convert units
    $("#unit-button").click(function () {
        var unit_pop = $('#unit-popup');
        if (unit_pop.css("display") == "none") {

            $(this).addClass("active");
            $('#unit-popup').fadeIn(200);
            convertUnits($("#quant1"), $("#quant2"));
        } else {
    ...
    $("#quant1").change(function () {
        convertUnits($(this), $("#quant2"));
    })

    $("#quant2").change(function () {
        convertUnits($(this), $("#quant1"));
    })

    $("#dropdown1").change(function () {
        convertUnits($("#quant2"), $(this).prev());
    })

    $("#dropdown2").change(function () {
        convertUnits($("#quant1"), $(this).prev());
    })

    function convertUnits(from, to) {
        var quant1 = from.val();
        var unit1 = from.next().val();
        var unit2 = to.next().val();

        var converted = convert(quant1).from(unit1).to(unit2);
        to.val(converted);
    }
